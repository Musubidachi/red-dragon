...
| `lib-analytics` | Scorers still mutate caller-provided note lists. Return-value based score+notes objects would better match the pure-function rule. Add integration/equivalence tests proving orchestrator output matches the underlying scorers and standalone classifier. | [lib-analytics/REVIEW.md](lib-analytics/REVIEW.md) |
...
| `lib-persistence` | Schwab tokens are persisted without documented encryption, integration coverage is thin, long text columns can truncate/reject large notes, and generated-id entities still expose positional all-args construction in some places. | [lib-persistence/REVIEW.md](lib-persistence/REVIEW.md) |
| `lib-backtest` | Production and backtest still duplicate `CandidateValidationInput` construction and risk-threshold literals. Determinism, verdict distribution, and average-score tests are still needed. | [lib-backtest/REVIEW.md](lib-backtest/REVIEW.md) |
...
delegation, validation OBSERVE reachability, persistence relationship/audit
migrations, validation-reason normalization, optimistic locking, and backtest
model immutability.
...
shared factories for production/backtest parity, secret storage, and tests that
protect behavior across module boundaries.
