...
Shared cross-module value objects are centralized in `lib-domain` under `dev.reddragon.domain.models`. Shared numeric helpers are centralized in `lib-math` under `dev.reddragon.math`. Module-owned DTOs stay close to their adapters or APIs.
...
| **L2 — Data Ingestion** | Normalize raw feeds into timestamped, source-tracked records | `lib-domain/models/`, `lib-ingestion/models/sec/`, `lib-marketdata/services/provider/`, `lib-marketdata/services/` |
...
| **L7 — Exit / Equilibrium Compression** | Has restoration completed or asymmetry compressed? | `lib-analytics/services/exit/` (scorer) + `lib-domain/models/exit/` (DTOs) — `EquilibriumCompressionScorer` returns `ExitSignal` (HOLD / TIGHTEN / SCALE_OUT / EXIT_NOW) via `POST /api/exit-signal` |
...
| `lib-domain` | Shared domain value objects and enums: candidates, bars, snapshots, validation results, verdict types | shared |
...
| `lib-execution` | Design docs for future Schwab order placement. Schwab OAuth and market-data token support currently live in `app`, `lib-marketdata`, and `lib-persistence`; broker order placement is not implemented. | (planned) post-L5 |
...
* `lib-domain/models/TradeCandidate.java` — the normalized record every L1/L2 source emits
...
3. `lib-domain/src/main/java/dev/reddragon/domain/models/TradeCandidate.java` — the value object that flows through every layer
...
* **Shared foundations stay small.** `lib-domain` owns cross-module value objects; `lib-math` owns numeric helpers.
